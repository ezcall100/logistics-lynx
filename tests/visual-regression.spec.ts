...
test.describe('Design System v2.0 Visual Regression', () => {
    ...
    for (const portal of PORTALS) {
      test(`${portal} portal - landing page`, async ({ page }) => {
        ...
        await page.addInitScript((portalKey) => {
          // Note: In browser context, the theme should already be available
          if (typeof window !== 'undefined' && (window as any).applyPortalTheme) {
            (window as any).applyPortalTheme(portalKey);
          }
        }, portal);
  ...
  test.describe('Performance Validation', () => {
    test('Page load performance', async ({ page }) => {
      const performanceMetrics: Array<{ portal: string; loadTime: number }> = [];
    ...
    test('Lighthouse scores', async ({ page }) => {
      ...
      for (const [pageName, scores] of Object.entries(lighthouseScores)) {
        ...
        // Log the page name for debugging purposes
        console.log(`Lighthouse scores for ${pageName}:`, scores);
      }
