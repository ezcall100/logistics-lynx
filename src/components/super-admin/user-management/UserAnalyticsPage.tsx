...
export const UserAnalyticsPage: React.FC = () => {
  ...
  const getMetricIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'user-plus': return <Users className="w-5 h-5" />;
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">User Analytics</h1>
            <p className="text-slate-600 dark:text-slate-400">Comprehensive insights into user behavior and engagement</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          {/* User Growth Chart */}
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/30 rounded-2xl p-6 shadow-xl">
          ...
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/30 rounded-2xl p-6 shadow-xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Device Usage</h3>
              <div className="flex items-center space-x-2">
              ...
              {deviceUsageData.map((data, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-slate-600 dark:text-slate-400 w-8">{data.value}%</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/30 rounded-2xl p-6 shadow-xl">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="px-4 py-2 text-sm text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all duration-200">
              View All
            </button>
          </div>
          ...
          <div className="space-y-4">
            {activities.map((activity, index) => (
              <motion.div
                ...
                transition={{ delay: index * 0.1 }}
                className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700/70 transition-all duration-200"
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <div className="text-sm font-medium text-slate-900 dark:text-white">{activity.user}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">{activity.action}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">{activity.device}</span>
                  </div>
                  <div className="flex items-center space-x-1">
        ...
        {selectedMetric && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="mt-8 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/30 rounded-2xl p-6 shadow-xl"
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{selectedMetric.name}</h2>
                  <p className="text-slate-600 dark:text-slate-400">Detailed analytics and insights</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Current Value</h3>
                ...
                <div className={`flex items-center space-x-1 ${getChangeColor(selectedMetric.changeType)}`}>
                  ...
                  <span className="text-sm font-medium">{Math.abs(selectedMetric.change)}% from last period</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">7-day average</span>
                    <span className="text-sm font-medium text-slate-900 dark:text-white">
                      {(selectedMetric.value * 0.95).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">30-day average</span>
                    <span className="text-sm font-medium text-slate-900 dark:text-white">
                      {(selectedMetric.value * 0.88).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Peak value</span>
                    <span className="text-sm font-medium text-slate-900 dark:text-white">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Insights</h3>
                <div className="space-y-3">
                  <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    <div className="flex items-center space-x-2">
                      <TrendingUp className="w-4 h-4 text-green-600 dark:text-green-400" />
                      <span className="text-sm font-medium text-green-700 dark:text-green-400">Positive trend</span>
                    </div>
                  ...
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium text-blue-700 dark:text-blue-400">Above target</span>
                    </div>
