...
const missingIcons = [
  ...
  'Minimize2', 'Maximize2', 'Handshake', 'Settings', 'Activity', 'Clock'
];

// Unused imports to remove
const unusedImports = [
  'Code', 'BookOpen', 'Radio', 'Link', 'CreditCard', 'Calculator', 
  'Truck', 'Fuel', 'Shield', 'ClipboardList', 'Wrench', 'UserCheck', 
  'Package', 'Warehouse'
];
...
function fixPortalFile(filePath) {
  ...
  // 1. Remove icon props from button elements
  content = content.replace(/icon={[^}]+}/g, '');
  modified = true;

  // 2. Fix component name issues
  ...
  // 4. Add missing state variables if they're referenced but not defined
  ...
  // 7. Clean up empty import statements and fix formatting
  ...
  // 8. Fix specific issues in MCPAgentsPortal
  if (filePath.includes('MCPAgentsPortal')) {
    ...
    content = content.replace(/const StatusIcon = getStatusIcon\([^)]+\);/g, '');
    modified = true;
  }
...
console.log('🔧 Fixing final portal build errors...\n');

portalFiles.forEach(fixPortalFile);

console.log('\n✅ All portal files have been processed!');
console.log('\n📋 Summary of fixes:');
console.log('- Removed icon props from button elements');
...
console.log('- Fixed StatusIcon issues in MCPAgentsPortal');
