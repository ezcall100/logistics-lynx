...
import {
  Building, Plus, Search, Edit, Trash, Eye, Users, 
  Mail, Phone, MapPin, Calendar, CheckCircle, X,
  Filter, Download, RefreshCw, TrendingUp, DollarSign,
...
const RealCompanyManagement: React.FC = () => {
  ...
  }, [companies, searchTerm, selectedIndustry, selectedSize, selectedStatus, sortField, sortDirection]);
  ...
  const getSizeColor = (size: Company['size']) => {
    switch (size) {
      case 'startup': return 'bg-purple-500/20 text-purple-400';
      case 'small': return 'bg-blue-500/20 text-blue-400';
      case 'medium': return 'bg-green-500/20 text-green-400';
      case 'large': return 'bg-yellow-500/20 text-yellow-400';
  ...
  const getPlanColor = (plan: Company['plan']) => {
    switch (plan) {
      ...
      case 'enterprise': return 'bg-green-500/20 text-green-400';
  ...
  const averageGrowth = companies.reduce((sum, company) => sum + company.growth, 0) / companies.length;
              ...
              {filteredCompanies.map((company) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex items-center">
                      <TrendingUp className={`w-4 h-4 mr-1 ${company.growth >= 0 ? 'text-green-400' : 'text-red-400'}`} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getPlanColor(selectedCompany.plan)}`}>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-400">Total Users</p>
                      <p className="text-2xl font-bold text-white">{selectedCompany.users.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Monthly Revenue</p>
                      <p className="text-2xl font-bold text-white">${selectedCompany.revenue.toLocaleString()}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center">
                      <TrendingUp className={`w-5 h-5 mr-2 ${selectedCompany.growth >= 0 ? 'text-green-400' : 'text-red-400'}`} />
                      <span className={`text-xl font-bold ${selectedCompany.growth >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  ...
                  <div>
                    ...
                    <p className="text-white">{new Date(selectedCompany.lastActive).toLocaleString()}</p>
                  </div>
