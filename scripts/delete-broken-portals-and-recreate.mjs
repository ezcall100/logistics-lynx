...
console.log('🗑️ Deleting broken portal files and recreating with working template...\n');
...
// Working template based on Customer Portal
const workingTemplate = (config) => `import React, { useState } from 'react';
...
function ${config.name.replace(/\s+/g, '')}() {
  ...
  const notifications = [
    { id: 1, title: 'New ${config.name.split(' ')[0].toLowerCase()} created', message: '${config.name} #1234 has been added', time: '5 minutes ago', type: 'info' },
    { id: 2, title: 'Status updated', message: 'Mark Johnson is now active', time: '1 hour ago', type: 'success' },
  ...
  const recentActivity = [
    { id: 1, action: 'New ${config.name.split(' ')[0].toLowerCase()} created', details: '${config.name} #1234 has been added to your network', time: '5 minutes ago', type: 'info', icon: Plus },
    { id: 2, action: 'Status updated', details: 'Mark Johnson is now active with ${config.name.split(' ')[0].toLowerCase()} #5678', time: '1 hour ago', type: 'success', icon: CheckCircle },
    ...
    { id: 4, action: 'New user registered', details: 'Beta Corp has been added to your network', time: 'Yesterday', type: 'info', icon: ${config.icon} },
...
// Delete and recreate all portal files
let successCount = 0;
let totalCount = portalConfigs.length;

console.log(`🚀 Processing ${totalCount} portals...\n`);
...
for (const config of portalConfigs) {
  ...
  try {
    ...
    // Write the new working template
    fs.writeFileSync(filePath, workingTemplate(config));
...
console.log(`\n🎉 Portal recreation complete!`);
console.log(`📊 Successfully processed: ${successCount}/${totalCount} portals`);
