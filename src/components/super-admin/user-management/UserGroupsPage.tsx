...
/**
 * User Groups Page - Visual Organization & Management
...
export const UserGroupsPage: React.FC = () => {
  ...
  const getGroupIcon = (iconName: string) => {
    switch (iconName) {
      case 'code': return <Users className="w-5 h-5" />;
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600 dark:text-green-400 flex items-center mt-1">
                  <UserPlus className="w-3 h-3 mr-1" />Across Groups
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-400">Public Groups</p>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-400">Private Groups</p>
            ...
            >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-slate-900 dark:text-white">{group.memberCount}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-600 dark:text-slate-400">Permissions</span>
                  <span className="text-sm font-semibold text-slate-900 dark:text-white">{group.permissions.length}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-600 dark:text-slate-400">Owner</span>
                  <span className="text-sm text-slate-500 dark:text-slate-400">{group.owner.name}</span>
                </div>
              </div>

              <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                <div className="flex items-center justify-between">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getGroupColor(group.color)}`}>
        ...
        {selectedGroup && (
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Group Members */}
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Group Members</h3>
                <div className="space-y-3">
              ...
              {/* Group Information */}
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Group Information</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Member Count</span>
                    <span className="text-sm font-semibold text-slate-900 dark:text-white">{selectedGroup.memberCount}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Permissions</span>
                    <span className="text-sm font-semibold text-slate-900 dark:text-white">{selectedGroup.permissions.length}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Owner</span>
                    <span className="text-sm text-slate-500 dark:text-slate-400">{selectedGroup.owner.name}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Created</span>
                    <span className="text-sm text-slate-500 dark:text-slate-400">{new Date(selectedGroup.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Last Updated</span>
                    <span className="text-sm text-slate-500 dark:text-slate-400">{new Date(selectedGroup.updatedAt).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
            </div>
