      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 dark:border-slate-700/50 w-full max-w-4xl max-h-[90vh] overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200/50 dark:border-slate-700/50">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100">System Status</h2>
                  <p className="text-sm text-gray-600 dark:text-slate-400">Real-time system metrics and performance</p>
                </div>
              </div>
              <button
                onClick={onClose}
                className="p-2 text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-xl transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {systemMetrics.map((metric, index) => (
                  <motion.div
                    ...
                    className="bg-white/70 dark:bg-slate-700/70 backdrop-blur-sm rounded-xl p-4 border border-gray-200/30 dark:border-slate-600/30 hover:shadow-lg transition-all duration-200"
                  >
                    <div className="flex items-center space-x-3">
                      <div className="p-2 bg-gray-100 dark:bg-slate-600 rounded-lg">
                        <metric.icon className="w-5 h-5 text-gray-600 dark:text-slate-300" />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900 dark:text-slate-100">{metric.name}</p>
                        <p className="text-lg font-bold text-gray-900 dark:text-slate-100">{metric.value}</p>
                      </div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50/50 dark:bg-slate-700/50 rounded-xl">
                      ...
                      <div className="flex-1">
                        <p className="text-sm text-gray-900 dark:text-slate-100">{incident.message}</p>
